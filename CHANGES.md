...
- `src/pages/` — `LoginPage` (built out, see Auth below); `OnboardingVoiceActingPage` and `ScenarioSelectionPage` (built out, see Voice Acting Onboarding below); `ProfilePage` (built out, see Profile Page below); `RecordingPage` (built out, see In-App Audio Recording below); `DashboardPage`, `FeedbackPage` (still stubs).
...
## In-App Audio Recording (Phase 1B, Task 5)

**What:** `useAudioRecorder` (`client/src/hooks/useAudioRecorder.ts`) wraps `navigator.mediaDevices.getUserMedia` + `MediaRecorder`: states `idle → recording ⇄ paused → stopped`, exposing `start()`/`pause()`/`resume()`/`stop()`/`reset()`, the assembled `audioBlob`/`audioUrl` on stop, a live `elapsedMs` timer, the raw `MediaStream` (for the waveform), and a typed `error` (`permission-denied` / `no-device` / `unsupported` / `unknown`). `RecordingPage` fetches the scenario (`GET /api/scenarios/:id`) and the caller's `voice_acting_profile.goals` (direct Supabase read, same pattern as `ScenarioSelectionPage`'s onboarding guard), and lays out a two-column, desktop-only, 100%-height grid: scenario info (title, dimension badges, goal line, context, script) on the left, recording controls on the right. Right column renders `RecordingStatusIndicator` (dot + timer) and, only while actively recording, `WaveformCarousel` — a live mic-level meter. On stop, shows native `<audio controls>` playback with "Re-record" / "Submit for Feedback"; submit POSTs multipart FormData to `/api/sessions/voice-acting` and navigates to the feedback page on success (a non-OK response, expected until Task 6, surfaces an inline error with retry rather than crashing).

**Why:** This is Task 5 of `PLAN.md` — the recording UI a performer uses between scenario selection and feedback. `MediaRecorder` is browser-native (no client dependency) per the plan's existing rationale.

**Method chosen vs. alternatives:**
- **Waveform Carousel is DOM bars, not canvas.** A ring buffer of levels (from `AnalyserNode.getByteTimeDomainData`, RMS per frame) is written directly to a fixed array of `<div>` heights via refs (bypassing React re-renders), not drawn on a `<canvas>`. Avoids canvas 2D's inconsistent OKLCH color-string support across browsers, and matches the fact that nothing else in this codebase uses canvas — one fewer rendering paradigm to maintain, colors stay as ordinary Tailwind classes.
- **Waveform Carousel is a named, scoped exception to DESIGN.md's "one continuous animation" rule**, not a silent violation — documented in `DESIGN.md`'s Recording State Indicator section and the motion Don't. Gated strictly to the active-`recording` state (freezes on pause, gone on stop/idle) so it can't drift into a general motif. Justification: performers have no other way to confirm the mic is actually picking up sound before they commit 60 seconds to a take.
- **Goals text displays verbatim, unfiltered** ("Your goal: …" next to the dimension badges) rather than semantically filtered to what's "relevant" to the selected scenario. Relevance judgment needs an LLM call that doesn't exist on this page; Task 6's feedback pipeline already passes full profile + scenario together, so the actual weighting happens invisibly server-side. Faking relevance client-side (keyword matching, etc.) would be less honest than showing the performer their own words plainly. See `PLAN.md`'s Task 5 note.
- **Same-stream reuse for `MediaRecorder` and the waveform's `AnalyserNode`** — one `getUserMedia()` call, not two. `WaveformCarousel` receives the hook's live `stream` prop and creates its own `AudioContext`/`MediaStreamSource` off it; never connects the analyser to `audioCtx.destination`, avoiding feedback/echo.
- **Elapsed timer excludes paused time** by shifting a `startTimeRef` forward by the paused duration on `resume()`, rather than tracking accumulated segments — simplest correct implementation for a single start/pause/resume/stop cycle (no multi-segment history needed here).
- **Desktop-only scope** — documented as a `PLAN.md` Global Constraint and a new `PRODUCT.md` "Platform Support" section, specifically because this task introduces the app's first genuinely complex two-column layout; without writing the constraint down, a future session could reasonably assume it needed a mobile fallback and diverge from the actual (desktop-only, deferred-mobile) intent.

### Recording quirks / gotchas
...
**Method chosen vs. alternatives:**
- **Async, not a blocking request.** The frontend was already built to poll (`usePolling`, `FeedbackSkeleton`, `SessionStatus`), so returning `{session_id}` immediately and processing in the background matches it exactly.
- **Pipeline runs in a separate process (`ProcessPoolExecutor`), not `BackgroundTasks`.** `BackgroundTasks` (and any sync work in FastAPI's threadpool) runs *in the web process*; the pipeline is 20–60s of CPU-bound work that pegs every core and holds the GIL (`librosa.pyin`, CTranslate2 with `cpu_threads=0`), which starves the single-process `fastapi dev` event loop — every other request (feedback poll, dashboard, profile) hangs until the pipeline finishes, then drains at once. A `ProcessPoolExecutor(max_workers=1)` worker (owned by the app `lifespan`, `spawn` context) has its own interpreter/GIL/CPU scheduling, so the web process stays responsive. Still no Celery/queue — MVP is single-process; a mid-run restart orphaning a `processing` row is acceptable (user re-records).
...
- **All handlers + services stay sync `def`** — matches every existing router; the underlying libs (supabase-py, faster-whisper, librosa) are all sync, so `async` would be a lie. The handlers' own supabase calls are short and fine in FastAPI's threadpool; only the long CPU-bound pipeline is pushed to a separate process (see above).
...
- Pipeline pool uses `spawn` (not `fork`) deliberately — Linux defaults to `fork`, and forking the threaded uvicorn process can deadlock. The `spawn` child re-imports `routers.sessions` (import-safe; `seed.py` is not in that graph, so no re-seed).
- `max_workers=1` serializes submissions — a second recording submitted while one is processing waits for the first to finish. The worker's first job also pays the whisper cold-load (~10s + one-time ~140MB download) while the session sits in `processing`.
- Worker-process death (OOM/segfault/`BrokenProcessPool`) leaves the session in `processing` forever — `run_pipeline`'s own `except` only covers in-process errors. `_log_worker_crash` (a future done-callback) logs it; there's no auto-retry.
